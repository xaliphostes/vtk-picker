 ...
 */
export function vtkPolyDataFromGocadTSurf(text, opts = {}) {
    const { computeNormals = false, shareAtomPoints = true, noDataToNaN = true, } = opts;
    // --- metadata containers ---
    let propertyNames = [];
    let propertyUnits = [];
    let propertyKinds = [];
    let propertySizes = []; // ESIZES
    let noDataValuesList = null; // can be one per property
    // Parse PROPERTY_CLASS_HEADER blocks to capture units/kind by property name
    const headerUnits = new Map();
    const headerKinds = new Map();
    // --- geometry containers ---
    const idToIndex = new Map(); // GOCAD vertex id -> VTK point index
    const pts = [];
    const tris = [];
    ...
    let propBuffers = null; // length = nProps; each holds flat values (components handled below)
    let nPropComponents = 0; // sum of ESIZES (or nProps if all size=1)
    let propCompOffsets = []; // for multi-component properties: running offsets
    ...
    const ensurePropBuffers = (maybeValueCount) => {
        if (propBuffers)
            return;
    ...
    const setPropValuesForPoint = (ptIdx, values) => {
        if (!propBuffers) {
    ...
    const parsePropertyClassHeader = (line, idx) => {
        // Example:
        ...
        while (i < lines.length) {
            ...
            if (unit)
                headerUnits.set(propName, unit[2]);
            const kind = /^(kind|kind:)\s*:\s*(.+)$/i.exec(L);
            if (kind)
                headerKinds.set(propName, kind[2]);
            i++;
        }
        return i;
    };
            ...
            if (parts.length >= 5) {
                ...
                if ([id, x, y, z].every(Number.isFinite)) {
                    ...
                    if (parts.length > 5) {
                        ...
                        if (propertyNames.length && !propBuffers)
                            ensurePropBuffers();
                    }
                }
            }
    ...
    if (pts.length === 0)
        throw new Error('TSurf parse: no vertices found (need VRTX/PVRTX).');
    if (tris.length === 0)
        throw new Error('TSurf parse: no triangles found (need TRGL).');
    ...
    if (noDataToNaN && propBuffers && noDataValuesList) {
        ...
        const perPropNoData = noDataValuesList.length === propertyNames.length
            ? noDataValuesList
    ...
    if (propBuffers && propertyNames.length) {
        for (let p = 0; p < propertyNames.length; p++) {
            ...
            da.unit = propertyUnits[p];
            da.kind = propertyKinds[p];
    ...
    const properties = propertyNames.map((name, i) => ({
        ...
        noData: noDataValuesList && (noDataValuesList.length === propertyNames.length
            ? noDataValuesList[i]
            : noDataValuesList[0]),
    }));
    return {
